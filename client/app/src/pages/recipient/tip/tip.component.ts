...
export class TipComponent implements OnInit {
  ...
  @ViewChild("tab1") tab1!: TemplateRef<
    TipUploadWbFileComponent | TipCommentsComponent
  >;
  @ViewChild("tab2") tab2!: TemplateRef<
    TipUploadWbFileComponent | TipCommentsComponent
  >;
  @ViewChild("tab3") tab3!: TemplateRef<
    TipUploadWbFileComponent | TipCommentsComponent
  >;
  ...
  showEditLabelInput7: boolean;
  showEditLabelInput8: boolean;
  active: string;
  loading = true;
  ...
  loadTipDate() {
    ...
    this.redactOperationTitle =
      this.translateService.instant("Mask") +
      " / " +
      this.translateService.instant("Redact");
    const requestObservable: Observable<any> = this.httpService.receiverTip(
      this.tip_id
    );
    this.loading = true;
    ...
    requestObservable.subscribe({
      next: (response: RecieverTipData) => {
        ...
        this.showEditLabelInput7 = this.tip.label7 === "";
        this.showEditLabelInput8 = this.tip.label8 === "";

        this.preprocessTipAnswers(this.tip);
        this.tip.submissionStatusStr = this.utils.getSubmissionStatusText(
          this.tip.status,
          this.tip.substatus,
  ...
  openGrantTipAccessModal(): void {
    this.utils.runUserOperation("get_users_names", {}, false).subscribe({
      next: (response) => {
        ...
        this.appDataService.public.receivers.forEach(
        ...
        modalRef.componentInstance.confirmFun = (receiver_id: Receiver) => {
          ...
          this.httpService
            .tipOperation(req.operation, req.args, this.RTipService.tip.id)
            .subscribe(() => {
              this.reload();
            });
        };
  ...
  openModalChangeState() {
    const modalRef = this.modalService.open(ChangeSubmissionStatusComponent, {
      backdrop: "static",
      keyboard: false,
    });
  ...
  openModalReopen() {
    const modalRef = this.modalService.open(ReopenSubmissionComponent, {
      backdrop: "static",
      keyboard: false,
    });
  ...
  updateSubmissionStatus() {
    const args = {
      ...
      substatus: this.tip.substatus ? this.tip.substatus : "",
    };
    this.httpService
      .tipOperation("update_status", args, this.tip.id)
      .subscribe(() => {
  ...
  tipNotify(enable: boolean) {
    this.httpService
      .tipOperation(
        "set",
        { key: "enable_notifications", value: enable },
        this.RTipService.tip.id
      )
      .subscribe(() => {
  ...
  setReminder() {
    ...
    modalRef.componentInstance.args = {
      ...
      reminder_date: this.utils.getPostponeDate(
        this.appDataService.contexts_by_id[this.tip.context_id].tip_reminder
      ),
      dateOptions: {
  ...
  tipPostpone() {
    ...
    modalRef.componentInstance.args = {
      ...
      operation: "postpone",
      contexts_by_id: this.appDataService.contexts_by_id,
      expiration_date: this.utils.getPostponeDate(
        this.appDataService.contexts_by_id[this.tip.context_id].tip_timetolive
      ),
      dateOptions: {
        minDate: this.utils.getMinPostponeDate(this.tip.expiration_date),
        maxDate: this.utils.getPostponeDate(
          Math.max(
            365,
  ...
  exportTip(tipId: string) {
    ...
    this.httpService.requestToken(param).subscribe({
      next: async (token) => {
        this.cryptoService.proofOfWork(token).subscribe((result: number) => {
          window.open(
            "api/recipient/rtips/" +
              tipId +
              "/export" +
